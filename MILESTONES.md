# Milestones — SQL Zero → Hero

Tick a box when you finish a module (concepts read + exercises done). I update this as we go.

**Overall progress:** 2 / 25 modules complete.

---

## Stage 1 — Foundations
- [x] Module 0 — Environment & Mental Model
- [x] Module 1 — SELECT Basics
- [ ] Module 2 — Filtering with WHERE
- [ ] Module 3 — Built-in Functions

## Stage 2 — Multiple Rows & Tables
- [ ] Module 4 — Aggregation & Grouping
- [ ] Module 5 — Joins
- [ ] Module 6 — Subqueries & CTEs
- [ ] Module 7 — Set Operators
- [ ] Module 8 — Window Functions

## Stage 3 — Building & Changing Databases
- [ ] Module 9 — Modifying Data (DML)
- [ ] Module 10 — Schema Design & DDL
- [ ] Module 11 — Constraints & Data Integrity
- [ ] Module 12 — Views
- [ ] Module 13 — Stored Procedures & Functions
- [ ] Module 14 — Triggers

## Stage 4 — Performance & Internals (the "hero" layer)
- [ ] Module 15 — Transactions & Concurrency
- [ ] Module 16 — Storage Internals
- [ ] Module 17 — Statistics & the Optimizer
- [ ] Module 18 — Indexes (Deep Dive)
- [ ] Module 19 — Reading Execution Plans
- [ ] Module 20 — Query Tuning & Best Practices

## Stage 5 — Professional Skills & Capstone
- [ ] Module 21 — Advanced T-SQL Toolkit
- [ ] Module 22 — Security
- [ ] Module 23 — Administration & Maintenance
- [ ] Module 24 — Capstone Projects

---

## Setup checklist
- [x] Ran `01_create_database.sql`
- [x] Ran `02_create_tables.sql`
- [x] Ran `03_seed_data.sql`
- [ ] (Optional) Ran `05_generate_large_data.sql` for perf modules

## Skills self-assessment (revisit at the end)
Rate 1–5 as you grow:
| Skill | Start | Now |
|---|---|---|
| Writing SELECT/JOIN queries | | |
| Aggregation & window functions | | |
| Designing schemas & constraints | | |
| Reading execution plans | | |
| Index design & tuning | | |
| Transactions & concurrency | | |
